...
import { actionUpgradeMerchant, actionVerifyMechant } from "~/store/merchant/action";

function MerchantColumn({ user, num }) {
	...
	const upgradeToAdmin = () => {
		dispatch(actionUpgradeMerchant(userDetails.id));
	};
	...
	return (
		<tr>
			...
			{button}
			<td>
				<DropdownAction  type="merchant"  id={userDetails.id} key={num + 1} />
			</td>
		</tr>
	);
}
